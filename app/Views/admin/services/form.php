...
<div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
    <form action="<?= isset($service) ? base_url('admin/services/update/' . $service['id']) : base_url('admin/services/create') ?>" method="POST">
        ...
        <div class="mb-6">
            ...
            <textarea name="description" rows="4" class="w-full px-4 py-2 rounded-lg border border-gray-200 outline-none focus:border-primary-500" placeholder="Tuliskan ringkasan layanan di sini..." required><?= old('description', $service['description'] ?? '') ?></textarea>
        </div>
